...
from openai import APIConnectionError, APIStatusError, APITimeoutError, \
    BadRequestError, OpenAIError, RateLimitError
...
class ChatInputList(list):
    """
    ChatInputList is a list of ChatInput objects. It is used to override the __str__
    method of list to return a string
...
class PromptResult(str):
    """
    PromptResult is the prompt tool output. This class substitutes the initial string
    output to avoid unintended parsing of roles for user input. The class has three
    properties:

    Original string: the previous rendered prompt result,
    Escaped string: the escaped prompt result string,
    Escaped mapping: the mapping of roles and uuids for the escaped prompt result
    string.
    """
    ...
    def merge_escape_mapping_of_prompt_results(self, **kwargs):
        prompt_result_escape_dict = Escaper.merge_escape_mapping_of_prompt_results(
            **kwargs)
    ...
    def merge_escape_mapping_of_flow_inputs(self, _inputs_to_escape: list, **kwargs):
        flow_inputs_escape_dict = Escaper.build_flow_inputs_escape_dict(
            _inputs_to_escape=_inputs_to_escape, **kwargs)
...
def validate_role(role: str, valid_roles: List[str] = None, escape_dict: dict = {}):
    ...
    if role not in valid_roles:
        ...
        error_message = (
            f"""The Chat API requires a specific format for prompt definition, and the
            prompt should include separate """
            f"""lines as role delimiters: {valid_roles_str}. Current parsed role
            '{unescaped_invalid_role}'"""
