...
|5| Document loading | The accelerator supports loading documents using Document Intelligence. These are loaded from a code repository. | Document Intelligence aligns with the technology stack agreed upon in [agreed architecture](../design/reference-architecture.md). However, the documents would have to be ingested from the GSK Data Lake. | Determine if we need to modify or add to `rag-experiment-accelerator/ingest_data` component to fit requirements. | We would need to develop the ingestion pipeline to source documents from the GSK Data Lake rather than the code repository. | High |
