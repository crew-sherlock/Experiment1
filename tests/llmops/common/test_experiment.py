...
def test_create_datasets_and_default_mappings():
    ...
    assert datasets[r_name].get_local_source() == str(CWD_PATH / "data" / r_source)
...
def test_create_eval_datasets_and_default_mappings():
    ...
    assert mapped_datasets[1].dataset.get_local_source() == str(
        CWD_PATH / "data" / r_source)

    assert not mapped_datasets[2].dataset.is_eval()
    assert mapped_datasets[2].dataset.get_local_source() == str(
        CWD_PATH / "data" / a_source)
